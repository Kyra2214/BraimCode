...
- **Item 1 — launcher equivalente ao bwrap no Android**: 🟡 investigado, não implementado. A viabilidade é condicional ao kernel, SELinux, ABI e fabricante; não existe garantia de `unshare(CLONE_NEWUSER)` em APK comum. O caminho padrão permanece `proot` com diagnóstico explícito, e um launcher de namespaces só deve ser codificado depois de um preflight não destrutivo e validação em emulador e dispositivo ARM64. Ver `docs/ANDROID_BWRAP_LAUNCHER_VIABILITY.md`.
