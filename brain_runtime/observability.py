...
class Observability:
    def __init__(self): self._lock = RLock(); self._spans: list[Span] = []; self._counters: dict[str, int] = {}
    ...
    def finish(self, span: Span, status: str = "OK", attributes: dict[str, Any] | None = None) -> Span:
        ...
        with self._lock:
            self._spans = [item if item.span_id != span.span_id else finished for item in self._spans]
            self._counters[f"span_finished:{status}"] = self._counters.get(f"span_finished:{status}", 0) + 1
        return finished
