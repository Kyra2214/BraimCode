...
class BrainPipeline:
    ...
    def _execute_core(self, task, run_id, session_id, actor, capability, provider, step_id, attempt=0, authorized=False, decision_id="approved"):
        ...
        self.events.append(run_id, session_id, task.task_id, EventType.AGENT_DISPATCHED, {"request_id": request.request_id, "capability": request.capability, "provider": provider, "binding_digest": binding.digest(), "policy_decision_id": decision.decision_id})
        ...
        if diagnosis is not None:
            ...
            if attempt < self.max_retries: self.events.append(run_id, session_id, task.task_id, EventType.CORRECTION_REQUESTED, {"step_id": step_id, "diagnostic": diagnosis}); self.events.append(run_id, session_id, task.task_id, EventType.RETRY, {"step_id": step_id, "attempt": attempt + 1}); return self._execute_core(task, run_id, session_id, actor, capability, provider, step_id, attempt + 1, authorized=authorized, decision_id=decision_id)
    ...
    def resume(self, approval_id: str, approver: str, approve: bool) -> list[ExecutionResult]:
        ...
        if pending is None:
            ...
            if event is not None and not decided:
                ...
                self.approvals.restore(payload); pending = {"task": task, "run_id": payload["run_id"], "session_id": payload.get("session_id", event.session_id), "actor": payload["actor"], "capability": payload["capability"], "provider": payload.get("provider", payload.get("resource", "")), "step_id": payload.get("step_id", new_id("step"))}; self._pending[approval_id] = pending
