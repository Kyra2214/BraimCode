...
class DefaultPromptBuilder:
    def build(self, task: TaskSpec, capability: str) -> str:
        # Keep untrusted context clearly delimited and never treat it as instructions.
...
class BrainPipeline:
    ...
    def _execute(self, task, run_id, session_id, actor, capability, provider, step_id, attempt=0, authorized=False, decision_id="approved"):
        ...
        if authorized:
            decision = SimpleNamespace(decision=Decision.ALLOW, reason="approval granted", decision_id=decision_id)
        self.events.append(run_id, session_id, task.task_id, EventType.POLICY_CHECKED,
                           {"decision": decision.decision.value, "reason": decision.reason, "decision_id": decision.decision_id})
        if decision.decision is Decision.ASK:
            approval = self.approvals.request(decision)
            self._pending[approval.approval_id] = {"task": task, "run_id": run_id, "session_id": session_id, "actor": actor,
                "capability": capability, "provider": provider, "step_id": step_id}
            self.events.append(run_id, session_id, task.task_id, EventType.APPROVAL_REQUESTED,
                               {"approval_id": approval.approval_id, "capability": capability})
        ...
        request = ExecutionRequest(new_id("request"), run_id, task.task_id, step_id, capability, prompt,
                                   {"provider": provider}, decision.decision_id)
        self.events.append(run_id, session_id, task.task_id, EventType.AGENT_DISPATCHED,
                           {"request_id": request.request_id, "capability": capability, "policy_decision_id": decision.decision_id})
        ...
        diagnosis = self.critic.diagnose(result, task, capability)
        if diagnosis is not None:
            ...
            if attempt < self.max_retries:
                self.events.append(run_id, session_id, task.task_id, EventType.CORRECTION_REQUESTED, {"step_id": step_id, "diagnostic": diagnosis})
                self.events.append(run_id, session_id, task.task_id, EventType.RETRY, {"step_id": step_id, "attempt": attempt + 1})
            ...
            return result
        self.events.append(run_id, session_id, task.task_id, EventType.VALIDATION_PASSED, {"request_id": request.request_id})
    ...
    def run(self, objective: str, session_id: str, actor: str = "brain") -> list[ExecutionResult]:
        ...
        self.events.append(run_id, session_id, task.task_id, EventType.PLAN_CREATED, {"plan_id": plan.plan_id, "steps": [step.step_id for step in plan.steps]})
    ...
    def resume(self, approval_id: str, approver: str, approve: bool) -> list[ExecutionResult]:
        ...
        approval = self.approvals.decide(approval_id, approver, approve, run_id=pending["run_id"], task_id=pending["task"].task_id,
                                          capability=pending["capability"], resource=pending["provider"])
        task, run_id, session_id = pending["task"], pending["run_id"], pending["session_id"]
        event = EventType.APPROVAL_GRANTED if approve else EventType.APPROVAL_DENIED
        self.events.append(run_id, session_id, task.task_id, event, {"approval_id": approval_id, "approver": approver})
