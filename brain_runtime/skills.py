...
class SkillRegistry:
    ...
    def validate_body(self, skill_id: str, base_path: str | Path) -> bool:
        ...
        root, body = Path(base_path).resolve(), (Path(base_path) / manifest.body_path).resolve()
        if root not in body.parents or not body.is_file() or body.is_symlink():
            return False
    ...
    def authorize(self, skill_id: str, available_permissions: Iterable[str]) -> bool:
        return set(self.get(skill_id).required_permissions).issubset(set(available_permissions))
