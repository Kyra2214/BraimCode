...
class SkillManifest:
    id: str; version: str; description: str; triggers: tuple[str, ...]; exclusions: tuple[str, ...] = (); body_path: str = ""; resources: tuple[str, ...] = (); tools: tuple[str, ...] = (); required_permissions: tuple[str, ...] = (); trust_level: str = "community"; license: str = ""; content_hash: str = ""; source_commit: str = ""; source_url: str = ""; signature: str = ""

class SkillRegistry:
    def __init__(self, trusted_levels: Iterable[str] = ("builtin", "verified"), quarantine_path: str | Path | None = None):
        self._trusted = set(trusted_levels); self._skills: dict[str, SkillManifest] = {}; self._revoked: set[str] = set(); self._lock = threading.RLock(); self.quarantine_path = Path(quarantine_path) if quarantine_path else None
    def register(self, manifest: SkillManifest) -> None:
        ...
        if manifest.id in self._revoked: raise PermissionError("skill is revoked")
    ...
    def revoke(self, skill_id: str, reason: str = "") -> None:
        with self._lock:
            self._revoked.add(skill_id); self._skills.pop(skill_id, None)
            if self.quarantine_path:
                self.quarantine_path.parent.mkdir(parents=True, exist_ok=True); with_file = self.quarantine_path.open("a", encoding="utf-8"); with_file.write(json.dumps({"skill_id": skill_id, "reason": reason}) + "\n"); with_file.close()
    def is_revoked(self, skill_id: str) -> bool: return skill_id in self._revoked
