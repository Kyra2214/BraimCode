...
class WorkflowEngine:
    ...
    def run(self, manifest: WorkflowManifest, run_id: str, idempotency_key: str, handler: Callable[[WorkflowNode], Any], owner: str = "local",
            ...
            backoff_seconds: float = 0.0, compensator: Callable[[WorkflowNode, dict[str, Any]], Any] | None = None,
            input_validator: Callable[[WorkflowNode, dict[str, Any]], bool] | None = None,
            output_validator: Callable[[WorkflowNode, dict[str, Any]], bool] | None = None) -> dict:
        with self._lock:
            ...
            if existing and existing.get("status") in {"running", "paused", "compensating"} and existing.get("owner") != owner and existing.get("lease_until", "") > now.isoformat():
            ...
            started_at = existing.get("started_at", now.isoformat()) if existing else now.isoformat()
            completed = set(existing.get("completed", [])) if existing else set(); attempts = existing.get("attempts", {}) if existing else {}
            outputs = existing.get("outputs", {}) if existing else {}; input_data = existing.get("outputs", {}) if existing else {}
            runtime_limit = timeout_seconds if timeout_seconds is not None else manifest.max_runtime_seconds
            def state(status: str, current: str | None = None, error: str | None = None) -> dict:
                return {"run_id": run_id, "workflow_id": manifest.workflow_id, "workflow_version": manifest.workflow_version,
                    "status": status, "owner": owner, "lease_until": (datetime.now(timezone.utc) + timedelta(seconds=lease_seconds)).isoformat(),
                    "started_at": started_at, "current_node": current, "completed": sorted(completed), "attempts": attempts,
                    "outputs": outputs, **({"error": error} if error else {})}
            for node in manifest.nodes:
                if node.node_id in completed: continue
                if any(dep not in completed for dep in node.dependencies):
                    result = state("blocked", node.node_id, "node dependencies are incomplete"); self._write_state(idempotency_key, result); return result
                if cancel_event is not None and getattr(cancel_event, "is_set", lambda: False)():
                    result = state("cancelled", node.node_id, "cancelled by caller"); self._write_state(idempotency_key, result); return self._compensate(result, nodes, outputs, completed, compensator, idempotency_key)
                if (datetime.now(timezone.utc) - datetime.fromisoformat(started_at)).total_seconds() > runtime_limit:
                    result = state("timed_out", node.node_id, "workflow runtime exceeded"); self._write_state(idempotency_key, result); return self._compensate(result, nodes, outputs, completed, compensator, idempotency_key)
                if node.input_schema and not all(key in input_data for key in node.input_schema):
                    result = state("failed", node.node_id, "node input validation failed"); self._write_state(idempotency_key, result); return result
                if input_validator and not input_validator(node, input_data):
                    result = state("failed", node.node_id, "node input validation failed"); self._write_state(idempotency_key, result); return result
                success = False; attempt = attempts.get(node.node_id, 0)
                self._write_state(idempotency_key, state("running", node.node_id))
                while attempt <= node.retry_limit:
                    if cancel_event is not None and getattr(cancel_event, "is_set", lambda: False)():
                        result = state("cancelled", node.node_id, "cancelled during node execution"); self._write_state(idempotency_key, result); return self._compensate(result, nodes, outputs, completed, compensator, idempotency_key)
                    ...
                    if payload.get("success", False) and valid_output:
                        success = True; completed.add(node.node_id); outputs[node.node_id] = payload; input_data = {**input_data, **payload}; break
                    if attempt <= node.retry_limit and backoff_seconds > 0: time.sleep(min(backoff_seconds * (2 ** (attempt - 1)), 30.0))
                if not success:
                    result = state("failed", node.node_id, "node execution or output validation failed"); self._write_state(idempotency_key, result); return self._compensate(result, nodes, outputs, completed, compensator, idempotency_key)
    ...
    def _compensate(self, result: dict, nodes: dict[str, WorkflowNode], outputs: dict, completed: set[str], compensator: Callable | None, key: str) -> dict:
        if compensator and completed:
            ...
            result["status"] = "cancelled" if result.get("error", "").startswith("cancel") else result.get("status", "failed")
            self._write_state(key, result)
        return result
