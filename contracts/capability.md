...
O registro canônico é `com.brain.capability.CapabilityRegistry`. O registry consulta metadados e produz candidatos estruturais; ele **não** autoriza nem executa. Autorização continua no `PolicyBroker`, e toda execução externa deverá passar pelo `ActionGateway` quando essa fase for consolidada.
